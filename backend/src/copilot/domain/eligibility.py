...
CITIZENSHIP = re.compile(
    r"""(
        \b(?:must|required\s+to)\s+be\s+a?\s*(?:us|u\.s\.|united\s+states)\s+citizen\b
      | \b(?:us|u\.s\.|united\s+states)\s+citizenship\s+(?:is\s+)?(?:required|mandatory)\b
...
def screen_eligibility(description: str | None, *, desc_available: bool = True) -> Eligibility:
    ...
    citizenship_span = _first_match(CITIZENSHIP, description)
    if citizenship_span:
